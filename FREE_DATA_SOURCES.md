...
- `npm test`: 799 tests passed plus five preliminary suites; `git diff --check` passed. Thirteen new tests cover parsing, unit ambiguity, partial failure, cache/retry, stale/regressed timestamps, timeouts, oversized responses, attribution rendering, client lifecycle and the HTTP route.
- Public API responses were inspected through web retrieval. Direct outbound requests from the local execution environment did not complete successfully; real end-to-end delivery from the app is not verified here. The owner subsequently verified HTTP 200 and all metrics current on review server 4188. Browser review found an Illegal invocation error from host functions called with the client as receiver. Default fetch/timer wrappers now retain the global receiver, with a dedicated regression test. Visual/browser re-verification of this correction is still required.
