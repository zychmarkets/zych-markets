...
const { createHttpServer, radarEventsMeta } = require('../server/http-server.js');
...
const radarEvents = require('../js/services/radar-event.js');
...
test('two same-market alerts trigger once per crossing without duplicate history or subscriptions',async()=>{for(const mode of ['once','recurring']){const f=await fixture(),first=(await f.runner.create(definition({type:'price',operator:'below',value:100},mode))).alert,second=(await f.runner.create(definition({type:'price',operator:'below',value:101},mode))).alert;assert.equal(f.runner.diagnostics().subscriptions,1);await f.runner.handle(ticker(102));await f.runner.handle(ticker(99));for(const price of [98,97,96,95,94])await f.runner.handle(ticker(price));assert.equal(f.runner.events().length,2);assert.equal(new Set(f.runner.events().map(event=>event.id)).size,2);assert.deepEqual(new Set(f.runner.events().map(event=>event.alertId)),new Set([first.id,second.id]));if(mode==='recurring'){f.runner.alerts.forEach(alert=>{alert.lastTriggeredAt=0});await f.runner.handle(ticker(102));await f.runner.handle(ticker(99));assert.equal(f.runner.events().length,4);assert.equal(new Set(f.runner.events().map(event=>event.id)).size,4)}else{await f.runner.handle(ticker(102));await f.runner.handle(ticker(99));assert.equal(f.runner.events().length,2)}await f.close()}});
...
test('HTTP API health, CRUD, validation and malformed payload', async () => {
  ...
  const app = await createServerApp({ config, logger: silent, transport, universe }); const address = await app.listen(), origin=`http://127.0.0.1:${address.port}`,base = `${origin}/api`;
  ...
  response=await fetch(`${base}/radar/events`);let radarFeed=await response.json();assert.equal(response.status,200);assert.deepEqual(radarFeed.events,[]);assert.equal(radarFeed.limit,50);assert.deepEqual(radarFeed.meta.coverage,{status:'HEALTHY',expectedExchanges:[],healthyExchanges:[]});assert.equal(radarFeed.meta.storeSize,0);assert.equal(typeof radarFeed.meta.generatedAt,'number');assert.equal(Array.isArray(radarFeed.meta.reasonCodes),true);
  const radarEvent=createUnifiedEvent({detectorId:'test',detectorVersion:'1',eventType:'TEST',market:{marketId:'binance:spot:BTCUSDT',exchange:'binance',marketType:'spot',symbol:'BTCUSDT',baseAsset:'BTC',quoteAsset:'USDT'},timeframe:'1h',eventTimestamp:Date.now(),observedAt:Date.now(),deepLink:{marketId:'binance:spot:BTCUSDT',exchange:'binance',marketType:'spot',symbol:'BTCUSDT',timeframe:'1h',eventTimestamp:Date.now()}},{id:'api-event'});app.eventStore.add(radarEvent);response=await fetch(`${base}/radar/events?exchange=binance&limit=1`);radarFeed=await response.json();assert.equal(radarFeed.events[0].eventId,'api-event');assert.equal(radarFeed.limit,1);assert.equal(radarFeed.meta.storeSize,1);assert.equal(radarFeed.meta.lastPublishedEventTimestamp,null);assert.equal(radarFeed.meta.lastHealthyProcessingTimestamp,null);assert.equal((await fetch(`${base}/radar/events?limit=9999`)).status,400);assert.equal((await fetch(`${base}/radar/events?exchange=kraken`)).status,400);assert.equal((await fetch(`${base}/radar/events?symbol=btc/usdt`)).status,400);assert.equal((await fetch(`${base}/radar/events/api-event`)).status,200);assert.equal((await fetch(`${base}/radar/events/missing`)).status,404);
  const browserFeed=new radarEvents.Feed({fetchImpl:(url,options)=>fetch(`${origin}${url}`,options)});await browserFeed.refresh();const selection=radarEvents.resolveSelection(browserFeed.state.events[0],[{id:'binance:spot:BTCUSDT',marketId:'binance:spot:BTCUSDT',exchange:'binance',marketType:'spot',symbol:'BTCUSDT',enabled:true}],{supportedTimeframes:['1h'],currentTimeframe:'5m'});assert.equal(selection.context.marketId,'binance:spot:BTCUSDT');assert.equal(selection.context.timeframe,'1h');browserFeed.stop();
...
test('Radar events endpoint reports an unavailable event store honestly',async()=>{const httpServer=createHttpServer({runner:{diagnostics(){return{}},list(){return[]},events(){return[]}},storage:{status(){return{}}},notifier:{},config:{root:path.resolve(__dirname,'..'),production:false,allowedOrigins:[]},logger:silent});await new Promise(resolve=>httpServer.server.listen(0,'127.0.0.1',resolve));const address=httpServer.server.address(),response=await fetch(`http://127.0.0.1:${address.port}/api/radar/events`),payload=await response.json();assert.equal(response.status,503);assert.equal(payload.error.code,'RADAR_EVENTS_UNAVAILABLE');await new Promise(resolve=>httpServer.server.close(resolve))});
test('Radar event metadata exposes only the public lifecycle, freshness, store, and coverage contract',()=>{const meta=radarEventsMeta({now:()=>99,health:{lifecycle:{snapshot:()=>({state:'DEGRADED',reasonCodes:['PARTIAL_PROCESSING_AVAILABLE'],transitionedAt:80})}},radar:{stats:{lastHealthyProcessingTimestamp:90,secretDetectorState:'hidden'}},qualification:{stats:{lastPublishedEventTimestamp:91,qualifiedCount:12}},eventStore:{size:7},universe:{health:()=>({coverage:{status:'DEGRADED',expectedExchanges:['binance','bybit','okx'],healthyExchanges:['binance','okx'],exchanges:{binance:{error:'private'}}}})}});assert.deepEqual(meta,{generatedAt:99,lifecycle:'DEGRADED',reasonCodes:['PARTIAL_PROCESSING_AVAILABLE'],lastHealthyProcessingTimestamp:90,lastPublishedEventTimestamp:91,storeSize:7,coverage:{status:'DEGRADED',expectedExchanges:['binance','bybit','okx'],healthyExchanges:['binance','okx']}})});
