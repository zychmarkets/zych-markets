...
test('push manager keeps the backend on the current deployment port',()=>{const button={dataset:{},setAttribute(){},disabled:false};const controller=new PushManagerController({button,location:{protocol:'http:',hostname:'localhost',port:'5173'}});assert.equal(button.textContent,'Push OFF');assert.equal(controller.baseUrl,'/api');assert.equal(resolvePushApiBase({protocol:'http:',hostname:'127.0.0.1',port:'4178'}),'/api')});
