...
(async()=>{
  const directory=await fs.mkdtemp(path.join(os.tmpdir(),'zych-bingx-live-'));
  const app=await createServerApp({config:{...loadConfig({}),port:4181,dataDir:directory,radarEnabled:false,radarIngestionEnabled:false}});
  await app.runner.persist();
  const ticks=[],transport=app.transport.transports.bingx;
  ...
  localStorage.setItem('zych.exchange-workspace.v1',JSON.stringify({exchange:'bingx',marketId:'bingx:spot:BTC-USDT',asset:'BTC',quoteAsset:'USDT',timeframe:'1m'}));</script>`;
  ...
  await app.listen();console.log('ISOLATED_REAL_BINGX http://127.0.0.1:4181/ '+directory);
  let stopping=false;const stop=async()=>{if(stopping)return;stopping=true;await app.stop();assert.equal(path.dirname(directory),os.tmpdir());assert.ok(path.basename(directory).startsWith('zych-bingx-live-'));await fs.rm(directory,{recursive:true,force:true});console.log('TEMP_DATA_REMOVED; OWNER_DATA_NOT_TOUCHED');};
