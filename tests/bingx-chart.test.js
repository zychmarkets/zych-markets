...
test('BingX UI shares watchlist table/tile identity and filter, keeps shared Alerts/Radar enablement',()=>{
  ...
  const filter=html.match(/id="watchlist-exchange-filter"[\s\S]*?<\/select>/)[0];assert.deepEqual([...filter.matchAll(/value="([^"]+)"/g)].map(m=>m[1]),['all','binance','bybit','okx','bingx','coinbase']);
