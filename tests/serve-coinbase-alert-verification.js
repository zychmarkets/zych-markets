...
(async()=>{
  const directory=await fs.mkdtemp(path.join(os.tmpdir(),'zych-coinbase-live-'));
  const app=await createServerApp({config:{...loadConfig({}),port:4187,dataDir:directory,radarEnabled:false,radarIngestionEnabled:false}});
  await app.runner.persist();
  const ticks=[],transport=app.transport.transports.coinbase;
  ...
  localStorage.setItem('zych.watchlist.v1',JSON.stringify([{key:'coinbase:spot:BTC-USD',marketId:'coinbase:spot:BTC-USD',exchange:'coinbase',marketType:'spot',symbol:'BTC-USD',asset:'BTC',baseAsset:'BTC',quoteAsset:'USD'}]));
  localStorage.setItem('zych.exchange-workspace.v1',JSON.stringify({exchange:'coinbase',marketId:'coinbase:spot:BTC-USD',asset:'BTC',quoteAsset:'USD',timeframe:'1m'}));</script>`;
  ...
  await app.listen();console.log('ISOLATED_REAL_COINBASE http://127.0.0.1:4187/ '+directory);
  let stopping=false;const stop=async()=>{if(stopping)return;stopping=true;await app.stop();assert.equal(path.dirname(directory),os.tmpdir());assert.ok(path.basename(directory).startsWith('zych-coinbase-live-'));await fs.rm(directory,{recursive:true,force:true});console.log('TEMP_DATA_REMOVED; OWNER_DATA_NOT_TOUCHED');};
