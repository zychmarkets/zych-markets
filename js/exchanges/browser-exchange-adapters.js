(function (global) {
  ...
  const snapshot = (price, change24h, change, high, low, quoteVolume24h, snapshotTimestamp, receivedAt,meta={}) => data.snapshot({price,change24h,change,high,low,quoteVolume24h,snapshotTimestamp,sourceTimestamp:snapshotTimestamp,receivedAt,...meta});
  ...
  class BinanceBrowserAdapter {
    ...
    async allSnapshots(signal) { const rows = await timeoutFetch(`${this.restBase}/ticker/24hr`, signal,10000,this.fetchImpl); const receivedAt=this.now(); return rows.map(row => ({ marketId: `${this.id}:spot:${row.symbol}`, symbol: row.symbol, ...this.normalizeTicker(row,receivedAt) })); }
    async snapshots(markets, signal) { if(markets.length>100){const wanted=new Set(markets.map(item=>item.id));return Object.fromEntries((await this.allSnapshots(signal)).filter(row=>wanted.has(row.marketId)).map(({marketId,symbol,...value})=>[marketId,value]))}const symbols = markets.map(item => item.symbol), rows = await timeoutFetch(`${this.restBase}/ticker/24hr?symbols=${encodeURIComponent(JSON.stringify(symbols))}`, signal,10000,this.fetchImpl); const receivedAt=this.now(); return Object.fromEntries(rows.map(row => [`${this.id}:spot:${row.symbol}`, this.normalizeTicker(row,receivedAt)])); }
  ...
  class BybitBrowserAdapter {
    ...
    async allSnapshots(signal) { const value=await timeoutFetch(`${this.restBase}/tickers?category=spot`,signal,10000,this.fetchImpl),sourceTimestamp=data.timestamp(value.time),receivedAt=this.now(),result=this.unwrap(value);return (result.list||[]).map(row=>({marketId:`${this.id}:spot:${row.symbol}`,symbol:row.symbol,...data.snapshot({...this.normalizeTicker(row,receivedAt),snapshotTimestamp:sourceTimestamp,sourceTimestamp,receivedAt})})) }
    async snapshots(markets,signal) {const value=await timeoutFetch(`${this.restBase}/tickers?category=spot`,signal,10000,this.fetchImpl),sourceTimestamp=data.timestamp(value.time),receivedAt=this.now(),result=this.unwrap(value),wanted=new Map(markets.map(item=>[item.symbol,item.id]));return Object.fromEntries((result.list||[]).filter(row=>wanted.has(row.symbol)).map(row=>[wanted.get(row.symbol),data.snapshot({...this.normalizeTicker(row,receivedAt),snapshotTimestamp:sourceTimestamp,sourceTimestamp,receivedAt})]))}
  ...
  class OkxBrowserAdapter {
    ...
    async allSnapshots(signal) { const rows = this.unwrap(await timeoutFetch(`${this.restBase}/market/tickers?instType=SPOT`, signal,10000,this.fetchImpl)),receivedAt=this.now(); return rows.map(row => ({ marketId: `${this.id}:spot:${row.instId}`, symbol: row.instId, ...this.normalizeTicker(row,receivedAt) })); }
    async snapshots(markets, signal) { const rows = this.unwrap(await timeoutFetch(`${this.restBase}/market/tickers?instType=SPOT`, signal,10000,this.fetchImpl)), receivedAt=this.now(),wanted = new Map(markets.map(item => [item.symbol, item.id])); return Object.fromEntries(rows.filter(row => wanted.has(row.instId)).map(row => [wanted.get(row.instId), this.normalizeTicker(row,receivedAt)])); }
  ...
  class BingxBrowserAdapter {
    ...
    normalizeTicker(row,receivedAt=this.now(),meta={}) { const source=data.number(row?.closeTime);return data.snapshot({...snapshot(row?.lastPrice,bingxPercent(row?.priceChangePercent),row?.priceChange,row?.highPrice,row?.lowPrice,row?.quoteVolume,source??receivedAt,meta.upstreamReceivedAt??receivedAt,{cacheStoredAt:meta.cacheStoredAt,cacheHit:meta.cacheHit}),sourceTimestamp:source}); }
    async allSnapshots(signal) { const response=await this.request(this.tickerPath,signal),receivedAt=this.now(),rows=this.unwrap(response),cacheMeta=response.__reliability||{},snapshots=rows.filter(row=>bingxPair(row)).map(row=>({marketId:`${this.id}:spot:${row.symbol}`,symbol:row.symbol,...this.normalizeTicker(row,receivedAt,cacheMeta)}));return [...new Map(snapshots.map(item=>[item.marketId,item])).values()]; }
  ...
  class CoinbaseBrowserAdapter {
    ...
    async allSnapshots(signal){const value=await this.products(signal);return value.products.filter(row=>!coinbase.unsupportedReason(row)).map(row=>({...coinbase.snapshot(row,value.upstreamReceivedAt??value.receivedAt),cacheStoredAt:data.timestamp(value.cacheStoredAt),cacheHit:value.cacheHit===true}));}
