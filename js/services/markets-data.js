(function(global){
  ...
  const item=(market,value={},watchlisted=false)=>({marketId:market.id||market.marketId,exchange:market.exchange,marketType:market.marketType,nativeSymbol:market.nativeSymbol||market.symbol,symbol:market.nativeSymbol||market.symbol,baseAsset:market.baseAsset,quoteAsset:market.quoteAsset,displaySymbol:market.displaySymbol,...(market.searchAliases?.length?{searchAliases:market.searchAliases}:{}),price:number(value.price??value.lastPrice),change24h:number(value.change24h??value.changePercent),quoteVolume24h:number(value.quoteVolume24h??value.volume),snapshotTimestamp:number(value.snapshotTimestamp),receivedAt:number(value.receivedAt),sourceTimestamp:number(value.sourceTimestamp),high24h:number(value.high24h??value.high),low24h:number(value.low24h??value.low),baseVolume24h:number(value.baseVolume24h),availability:value.availability||null,provenance:value.provenance||null,watchlisted:Boolean(watchlisted)});
