(function(global,factory){
  ...
  if(typeof module==='object'&&module.exports)module.exports=api;else global.ZychInstruments=api;
})(typeof window!=='undefined'?window:globalThis,function(adapters){
  'use strict';
  if(!adapters)throw new Error('Adapter v2 registry is required');
  const PRODUCT_EXCHANGES=adapters.exchangeIds,MARKET_TYPES=new Set([...new Set(adapters.registry.flatMap(row=>Object.keys(row.marketTypes)))]);
  const clean=value=>String(value||'').trim();
  const instrumentId=value=>adapters.canonicalId(clean(value?.exchange).toLowerCase(),clean(value?.marketType||'spot').toLowerCase(),clean(value?.nativeSymbol||value?.symbol).toUpperCase());
  function normalize(value){
    ...
    if(!PRODUCT_EXCHANGES.includes(exchange)||!MARKET_TYPES.has(marketType)||!baseAsset||!quoteAsset||!nativeSymbol)return null;
  ...
  return{PRODUCT_EXCHANGES,MARKET_TYPES,instrumentId,normalize,search};
});
