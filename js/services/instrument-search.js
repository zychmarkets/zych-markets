(function(global){
  ...
  function search(instruments,query,{limit=100}={}){
    ...
    const normalized=(instruments||[]).map(normalize).filter(Boolean),hasBaseMatch=normalized.some(instrument=>instrument.baseAsset.includes(term)),includeQuoteMatches=['USD','USDT','USDC','FDUSD'].includes(term),quotePriority={USDT:0,USDC:1,FDUSD:2};
