...
class RadarEventPipeline{
  ...
  publishCandidate(candidate){if(this.stopped)return false;const validation=validateCandidateEvent(candidate);if(!validation.valid){this.stats.validationFailures++;return false}this.stats.candidateCount++;if(this.queue.length>=this.queueLimit){this.queue.shift();this.stats.droppedEventsCount++}this.queue.push(candidate);this.drain();return true}
  ...
  async stop(){this.stopped=true;await this.idle();this.queue=[]}
}
