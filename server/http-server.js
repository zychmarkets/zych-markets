...
const {healthReliability}=require('./reliability-diagnostics');
const {withReliability}=require('../js/services/reliability-reducer');

const TYPES = { '.html': 'text/html; charset=utf-8', '.js': 'text/javascript; charset=utf-8', '.css': 'text/css; charset=utf-8', '.json': 'application/json; charset=utf-8', '.svg': 'image/svg+xml' };
const PUBLIC_FILES = new Set(['index.html','app.js','style.css','sw.js','third-party/lightweight-charts-5.0.9/lightweight-charts.standalone.production.js']);
const CSP = "default-src 'self'; script-src 'self'; style-src 'self'; connect-src 'self' https://api.binance.com https://api.bybit.com https://www.okx.com wss://stream.binance.com:9443 wss://stream.bybit.com wss://ws.okx.com:8443 wss://open-api-ws.bingx.com wss://advanced-trade-ws.coinbase.com https://api.kraken.com wss://ws.kraken.com; img-src 'self' data:; media-src 'self' blob:; worker-src 'self'; object-src 'none'; base-uri 'self'; frame-ancestors 'none'; form-action 'self'";
...
function createBingxPublicProxy({fetchImpl=globalThis.fetch,base='https://open-api.bingx.com',timeoutMs=8000,cacheTtlMs=5000,now=Date.now}={}){
  ...
  return async(key,params)=>{
    ...
    const cached=cache.get(cacheKey);if(cached&&now()-cached.storedAt<cacheTtlMs)return {...cached.value,__reliability:{cacheHit:true,cacheStoredAt:cached.storedAt,upstreamReceivedAt:cached.upstreamReceivedAt}};
    ...
    const request=(async()=>{const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),timeoutMs);try{
      ...
      const value=await response.json(),upstreamReceivedAt=now(),rows=key==='catalog'?value?.data?.symbols:value?.data;if(Number(value?.code)!==0||!Array.isArray(rows))throw new Error('BingX upstream returned an invalid envelope');
      if(!history){const storedAt=now();cache.set(cacheKey,{storedAt,upstreamReceivedAt,value});return {...value,__reliability:{cacheHit:false,cacheStoredAt:storedAt,upstreamReceivedAt}}}return value;
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (pathname.startsWith('/api/')) {
        ...
        if (pathname === '/api/push/subscribe' && req.method === 'POST') { const record = await storage.savePushSubscription(await body(req)); if (!record) return error(res, 422, 'INVALID_SUBSCRIPTION', 'Invalid push subscription.'); if(record.error==='PUSH_SUBSCRIPTION_LIMIT')return error(res,429,record.error,'This installation has reached its 100 device limit. Remove unused subscriptions first.'); logger.info('push_subscription_created', { endpointHost: new URL(record.endpoint).host }); return send(res, 201, { subscribed: true }); }
