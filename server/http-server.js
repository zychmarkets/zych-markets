...
function createHttpServer({ runner, storage, notifier, universe = null, eventStore = null, eventPipeline = null, radar = null, config, logger, startedAt = Date.now() }) {
  let accepting = true;
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (pathname.startsWith('/api/')) {
        ...
        if (pathname === '/api/radar/health' && req.method === 'GET') return universe ? send(res,200,{universe:universe.health(),ingestion:radar?.diagnostics()||{enabled:false,running:false},marketState:radar?.store?.diagnostics()||{totalStateEntries:0,COMPLETE:0,WARMING_UP:0,STALE:0,DEGRADED:0},recovery:radar?.recovery?.diagnostics()||null,detectors:radar?.registry?.diagnostics()||[],eventPipeline:eventPipeline?.diagnostics()||null}) : error(res,503,'RADAR_UNAVAILABLE','Radar is not initialized.');
