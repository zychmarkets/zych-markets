...
const applyCors=(req,res,config)=>{const origin=req.headers.origin;if(!origin)return true;let own=false;try{own=new URL(origin).host===req.headers.host}catch{}const allowed=own||config.allowedOrigins?.includes(origin);if(!allowed)return false;res.setHeader('access-control-allow-origin',origin);res.setHeader('vary','Origin');res.setHeader('access-control-allow-methods','GET,POST,PATCH,DELETE,OPTIONS');res.setHeader('access-control-allow-headers','content-type');return true};
