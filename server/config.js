...
const ratio = (value, fallback) => { const parsed=Number(value); return Number.isFinite(parsed)&&parsed>=0&&parsed<=1?parsed:fallback; };
...
function loadConfig(env = process.env, root = path.resolve(__dirname, '..')) {
  return Object.freeze({
    ...
    shutdownTimeoutMs: integer(env.ZYCH_SHUTDOWN_TIMEOUT_MS, 15000, 1000, 120000),
    ...
    radarReadinessMinCompleteRatio: ratio(env.RADAR_READINESS_MIN_COMPLETE_RATIO, .1),
    radarReadinessMaxHealthyAgeMs: integer(env.RADAR_READINESS_MAX_HEALTHY_AGE_MS, 180000, 10000, 3600000),
    ...
    radarBreadth: Object.freeze({enabled:env.RADAR_BREADTH_ENABLED!=='false',timeframe:['1m','5m','15m'].includes(String(env.RADAR_BREADTH_TIMEFRAME||'15m').toLowerCase())?String(env.RADAR_BREADTH_TIMEFRAME||'15m').toLowerCase():'15m',flatThresholdPct:number(env.RADAR_BREADTH_FLAT_THRESHOLD_PCT,.05,0),minCoveragePct:ratio(env.RADAR_BREADTH_MIN_COVERAGE_PCT,.8),intervalMs:integer(env.RADAR_BREADTH_INTERVAL_MS,10000,1000,300000),maxAgeMs:integer(env.RADAR_BREADTH_MAX_AGE_MS,30000,1000,3600000)}),
    radarScore: Object.freeze({enabled:env.RADAR_SCORE_ENABLED!=='false',modelVersion:env.RADAR_SCORE_MODEL_VERSION||'zych-score-v1',interpretationVersion:env.RADAR_INTERPRETATION_VERSION||'interpretation-v1',caps:Object.freeze({momentum:integer(env.RADAR_SCORE_MOMENTUM_CAP,30,0,100),volume:integer(env.RADAR_SCORE_VOLUME_CAP,25,0,100),confirmation:integer(env.RADAR_SCORE_CONFIRMATION_CAP,20,0,100),context:integer(env.RADAR_SCORE_CONTEXT_CAP,15,0,100),dataQuality:integer(env.RADAR_SCORE_DATA_QUALITY_CAP,10,0,100)}),momentumFullZ:number(env.RADAR_SCORE_MOMENTUM_FULL_Z,6,.5),volumeFullRelative:number(env.RADAR_SCORE_VOLUME_FULL_RELATIVE,4,1),volumeFullZ:number(env.RADAR_SCORE_VOLUME_FULL_Z,6,.5)}),
