...
async function createServerApp(options = {}) {
  ...
  const httpServer = createHttpServer({ runner, storage, notifier, universe, eventStore, eventPipeline, radar, qualification, breadth, scorePolicy, interpretationPolicy, config, logger });
  let stopped = false;
  return {
    config, logger, storage, transport, runner, universe, eventStore, eventPipeline, radar, qualification, breadth, scorePolicy, interpretationPolicy, server: httpServer.server,
