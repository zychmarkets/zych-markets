...
async function createServerApp(options = {}) {
  ...
  if(!breadth&&universe&&radar?.store&&config.radarBreadth?.enabled){breadth=new BreadthService({universe,store:radar.store,calculator:new BreadthCalculator(config.radarBreadth),enabled:true,intervalMs:config.radarBreadth.intervalMs,maxAgeMs:config.radarBreadth.maxAgeMs,logger});await breadth.start()}else await breadth?.start?.();
  const httpServer = createHttpServer({ runner, storage, notifier, universe, eventStore, eventPipeline, radar, qualification, breadth, config, logger });
  let stopped = false;
  return {
    config, logger, storage, transport, runner, universe, eventStore, eventPipeline, radar, qualification, breadth, server: httpServer.server,
    ...
    async stop() { if (stopped) return; stopped = true; httpServer.stopAccepting(); await new Promise(resolve => httpServer.server.listening ? httpServer.server.close(resolve) : resolve()); await breadth?.stop?.(); await radar?.stop(); await universe?.stop(); await eventPipeline.idle(); qualification.stop(); await eventPipeline.stop(); eventStore.stop(); await runner.stop(); logger.info('server_stopped'); }
  };
}
