...
async function createServerApp(options = {}) {
  ...
  const transport = options.transport || new MultiExchangeMarketTransport({ logger, transports: { binance: new BinanceMarketTransport({ restBase: config.binanceRestBase, wsBase: config.binanceWsBase, logger }), bybit: new BybitMarketTransport({ restBase: config.bybitRestBase, wsBase: config.bybitWsBase, logger }) } });
